...
public interface QuestChallengeApi {
    @Operation(summary = "회원별 퀘스트 챌린지 목록 조회 (자기자신 or 어드민)", description = "특정 회원이 참여한 퀘스트 챌린지 목록을 조회합니다.")
    ...
    // 추가 엔드포인트 예시 (Swagger 문서화)
    @Operation(summary = "퀘스트별 챌린지 목록 조회 (자기자신 or 어드민)", description = "특정 퀘스트에 대한 챌린지 목록을 조회합니다.")
    ResponseEntity<List<QuestWithLikeResponseDto>> getQuestChallengesByQuestId(
            @Parameter(description = "퀘스트 ID", example = "1") @PathVariable(name = "questId") Long questId,
            @Parameter(description = "완료 여부") @RequestParam(required = false) Boolean completed,
            Pageable pageable);

    @Operation(summary = "퀘스트 챌린지 조회 (자기자신 or 어드민)", description = "특정 퀘스트 챌린지의 상세 정보를 조회합니다.")
    ResponseEntity<QuestWithLikeResponseDto> getQuestChallengeById(
    ...
    ResponseEntity<QuestWithLikeResponseDto> createQuestChallenge(
    ...
    ResponseEntity<QuestWithLikeResponseDto> completeQuestChallenge(
            @Parameter(description = "퀘스트 챌린지 ID", example = "1") @PathVariable(name = "questChallengeId") Long questChallengeId);

    @Operation(summary = "퀘스트 챌린지 실패 처리 (자기자신 or 어드민)", description = "특정 퀘스트 챌린지를 실패 처리합니다.")
    ResponseEntity<QuestWithLikeResponseDto> failQuestChallenge(
            @Parameter(description = "퀘스트 챌린지 ID", example = "1") @PathVariable(name = "questChallengeId") Long questChallengeId);
}